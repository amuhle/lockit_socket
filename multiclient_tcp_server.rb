...
class MulticlientTCPServer
    ...
    def get_socket
        ...
        puts "----------1"
        ios = select( [@server]+@connections, nil, @connections, @timeout ) or return nil
        ...
        ios[0].each do |s| 
            ...
            s==@server or next 
            client = @server.accept or raise "server: incoming connection, but no client"
            @connections << client
            @verbose and puts "server: incoming connection no. #{@connections.size} from #{client.peeraddr.join(':')}"
        ...
        ios[0].each do |s|
            ...
            if s.eof?
                ...
                @verbose and puts "server: client closed #{s.peeraddr.join(':')}"
